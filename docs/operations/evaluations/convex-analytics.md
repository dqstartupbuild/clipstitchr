...
conversion tracking documented in `docs/integrations/analytics/posthog.md` and
...
- Rate-limited TikTok Events API forwarding in `docs/operations/security/rate-limits.md`.
...
- Use `identify` only after a privacy review confirms the allowed user ID shape
  and traits.
- Update the privacy page and `docs/integrations/analytics/*` before shipping.
...
- `docs/integrations/analytics/posthog.md` or a new analytics tracking plan: document whether
  Convex Analytics replaces or supplements PostHog.
- `docs/operations/security/rate-limits.md`: document ingest limits, cleanup, and retention.
