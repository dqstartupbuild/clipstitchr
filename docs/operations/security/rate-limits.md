...
| CLI queue add | `POST /api/cli/queue/stitches`, `POST /api/cli/queue/swipes` | Bearer-token authorized only. The Stitch route verifies the Stitch belongs to the CLI session owner and has a finished R2 video. The Swipe route verifies the Swipe belongs to the CLI session owner and has a saved rendered image. Both routes read the owner's saved Post Bridge key, verify selected or product-linked Post Bridge accounts, consume Post Bridge schedule quota before upload-byte quota and provider work, stream the saved media to Post Bridge without deleting the saved asset, create the queued post with `use_queue`, then consume `convexMetadataUpdate` when attaching the returned Post Bridge post reference to the Stitch or Swipe. Full Swipe carousel/video queueing remains dashboard-rendered; the CLI route only uses stored rendered media. All provider calls use the same API-key pacing guard as the dashboard. |
| CLI queue list | `GET /api/cli/queue/list` | Bearer-token authorized only. This read-only route still touches the external Post Bridge API, so it consumes the existing Post Bridge read limits before loading posts. It filters to queued Stitches and Swipes scheduled or queued within the next 24 hours and joins local source/product mapping when available. |
...
| Product enrichment, Hook Lab writing guardrails, and website import | `POST /api/settings/products`, `PATCH /api/settings/products/{id}` | 100/hour/user, burst 20; 2,000/30 days/user; global 5,000/hour. The route consumes this limit before Firecrawl website crawling and before the Replicate product enrichment call. Website import is capped at 15 Firecrawl pages per create/update. Product edits only re-crawl Firecrawl when the saved website URL changes. Hook Lab goal, tone, and avoid phrases are saved on the same product create/update path and do not add a separate provider call. Legacy winning examples pass through unchanged for rollback; new positive memory is saved as Ideas. |
| Hook Lab Idea analysis | `POST /api/hook-lab/ideas` and `POST /api/hook-lab/ideas/{id}/retry` | Idea creation first consumes `convexRecordSave`. Before a new provider job, the route consumes 30 analyses/day/user with burst 5, 1,000/day globally with burst 200 across 5 shards, and the shared provider-spend guard. Social sources additionally consume 15 imports/day/user with burst 3 and 300/day globally with burst 60 across 5 shards before Apify run creation. Each social attempt may stage one owner-private temporary R2 video only after those limits pass; the existing limits, 100 MiB hard cap, deterministic job-scoped key, and `finally` deletion bound that internal storage work, so it intentionally has no separate user-facing R2 bucket. A provider-job idempotency hit reports `alreadyReserved` and does not reserve the same work twice. HTTP rejection returns `429` with `Retry-After`, and an already-created Idea moves to `needs_attention` so it can be retried instead of remaining stuck as analyzing. |
| Hook Lab Idea use | `POST /api/hook-lab/ideas/{id}/use`, enforced inside `hookLabIdeaUses.create` before child jobs | 10 requested variants/day/user with burst 5, counted by the 1/3/5 variation request. The mutation also reserves Clipr writing and avatar-still quota per variant, eight generated-video seconds per variant, ten shared provider-spend units per variant, two Hook Lab asset saves per variant, and one Convex record per use plus one per variant before inserting any use/variant rows. |
| Hook Lab final asset saves | Reserved in `hookLabIdeaUses.create` before provider/media jobs | 20 objects/day/user with burst 10; 2,000/day globally with burst 400 across 5 shards. Two R2 objects are reserved per variant for the normalized opening video and its poster. The editable Stitch is a Convex record and has no rendered R2 video at creation. |
| Hook Lab Idea and Review metadata | `hookLabIdeas.create`, `hookLabIdeas.update`, `hookLabIdeas.archive`, `hookLabIdeas.retry`, `hookLabIdeas.remove`, `hookLabDefaults.set`, and `stitchrHookOptions` feedback mutations | Idea creates consume `convexRecordSave`; edits, archive/retry/default changes, selection, **Not for me**, and Undo consume `convexMetadataUpdate`; Idea deletion consumes `convexRecordDelete`. Saving a Review hook as an Idea uses the normal Idea create limit. No feedback action silently creates a Template. |
...
| Provider worker jobs | `npm run provider-worker` with `PROVIDER_WORKER_SECRET`; provider-only mutations such as `providerJobs.claimNextForProvider`, `automationTasks.claimNextForProvider`, `automationTasks.markProviderStatus`, `mediaJobs.create*FromProvider`, `providerWorkerLaunch.requestContinuation`, and provider `replicateJobs` writes | Provider work is claimed from Convex and no longer dispatched through protected Next.js Preview routes. Creating a manual `providerJobs` record or automation task schedules a coalesced Convex dispatch action that runs the Cloud Run provider job immediately, schedules a short 3-second follow-up when an immediate launch is coalesced, and schedules a coalesced delayed recovery dispatch 10 minutes after the launch target. Trusted API routes may call `workerDispatch.runWorkerFromApi` with `AUTOMATION_WORKER_SECRET` only after their own user-facing limits have been consumed; Stitchr Batch uses this to start the provider job after foreground hook planning while retaining a delayed fallback launch. Bounded provider worker runs request a worker-secret continuation launch when they process a full `maxJobs` batch. Hook Lab analysis uses explicit 30-second continuations while asynchronous Apify runs are nonterminal. The worker handles Hook Lab analysis/use, manual Swapr, manual Clipr, manual avatar-photo generation, upload video analysis, automated Stitch score analysis, automatic Stitchr overlay/caption/hashtag text, automatic Swapr create/finalize, automatic Clipr text/still/video, automatic avatar-photo generation, and automatic Swipr Pexels-photo/text draft generation. Manual routes consume their user limits before creating jobs; automation planners consume tool budgets before worker execution. |
| Media worker jobs | `mediaJobs.createUploadNormalization`, `mediaJobs.createCliprFinalizationFromProvider`, `mediaJobs.createSwaprFinalizationFromProvider`, `mediaJobs.createStitchrDraftFinalizationFromProvider`, Hook Lab variant finalization, existing automation media creators, `mediaWorkerLaunch.requestContinuation`, and `npm run media-worker` | Media jobs are worker-secret controlled. Creating a media job schedules a coalesced Convex dispatch action that runs the Cloud Run media job immediately, schedules a short 3-second follow-up when an immediate launch is coalesced, and schedules a coalesced delayed recovery dispatch 10 minutes after the launch target. Bounded media worker runs request a worker-secret continuation launch when they process a full `maxJobs` batch. The worker normalizes fallback video uploads when browser Media Bunny processing is unavailable, creates upload posters, saves uploaded clips, creates upload-analysis provider jobs, saves editable Stitchr drafts with generated captions, creates Stitch score provider jobs for automated Stitchr and user-triggered Stitchr Batch outputs, finalizes Hook Lab opening clips/Stitches, finalizes Swapr provider outputs, and finalizes Clipr outputs. Manual asset saves consume normal user-facing limits before job creation; Hook Lab reserves its dedicated final-save buckets before provider work; automatic final asset saves consume automatic asset save buckets: 20 saved assets/day/user; global 2,000/day. Stitchr Batch final asset saves consume separate Batch asset-save buckets. |
| Stitchr Batch generation | Signed-in `POST /api/stitchr/batch/generate` before provider work; can run at any time and uses `stitchrBatchPairHistory`, not automation pair history. The browser sends its IANA time zone, and the API computes the Batch date in that time zone with UTC fallback for missing or invalid values. Optional setup-Idea selection reuses owned saved recipe text/caption data and does not bypass Batch limits; the compatibility resolver can fall back to a legacy Template. Convex creates or repairs active tasks, schedules a delayed provider-worker fallback, and the API route directly dispatches the provider worker through Convex after foreground hook planning. No direct dispatch is attempted when Convex returns no active task IDs, such as an already completed daily batch. | 10 Stitchr Batch outputs/user/browser-local batch date; global 1,000/day; final Batch asset saves use the same owner-and-batch-date key plus the 1,000/day global asset-save bucket |
| Stitchr Batch hook planning | Signed-in `POST /api/stitchr/batch/generate` after durable batch task creation and before the foreground writing provider call | 20 hook-planning calls/day/user, burst 10; global 2,000/day, burst 500; shared provider bucket 10,000 units/hour, burst 2,000. Recipe-covered batches skip this limit because no writing provider call is made. Planner failures do not delete the already-created batch tasks; provider workers fall back to per-stitch writing for missing or failed plans. |
| Automatic Stitchr generation | Worker-only automation planner before provider work; scheduled planning respects the daily generation window and uses automation pair history. Optional setup-Idea allocations reuse owned saved recipe text/caption data and do not bypass automation budgets; legacy allocation IDs continue through fallback reads. | User can choose 3, 5, or 10 Stitchr outputs/day/product; global 1,000/day. The per-tool bucket is keyed by owner and product. |
...
Hook Lab writing preferences and ready Idea text blueprints are bounded prompt
resources and do not create provider work by themselves. The prompt-memory read
selects at most eight product/shared blueprints and omits their source text when
formatting Stitchr prompts. Creating or retrying an Idea that needs analysis
uses the dedicated analysis limits above. Using an Idea reserves every
requested variant's writing, avatar, generated-video, provider-spend,
asset-save, and Convex-save costs before provider jobs are inserted. Selecting,
saving, marking **Not for me**, or undoing one Review card affects only that
option and uses the shared Convex metadata/save limits; feedback does not call a
provider.
...
10. Reduce `hookLabSocialImport` and create two social-link Ideas as the same
    user. Confirm the second expensive request returns `429` with
    `Retry-After` before a new Apify Actor run is created.
11. Retry the same Hook Lab analysis idempotency key and confirm it does not
    reserve analysis or shared provider quota twice.
12. Reduce `hookLabIdeaUse`, request 3 or 5 variants, and confirm the mutation
    rejects the entire request before any use, variant, or provider job is
