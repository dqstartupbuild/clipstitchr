...
- Planner failure records failed hook-plan rows and returns the batch response.
...
planning, saving per-task planner output, and preserving rate-limit responses:

```bash
cd web
npx vitest run app/api/stitchr/batch/generate/route.test.ts lib/clipstitchr/utils/moreUtilitiesCoverage.test.ts
```
