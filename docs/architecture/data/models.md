...
Replicate's 1024-token minimum. Keep new writing calls on that helper so future
tools inherit the correct provider input shape.
...
| CLI AI guide generation | `CLI_DEMO_GUIDE_MODEL_ID` | `openai/gpt-5-mini` | Writes label-only walkthrough guides for `clipstitchr demo guide create` and `clipstitchr demo agent`. This uses the existing provider prediction path but has its own OpenAI text-model default. |
...
| Clipr Demo remix video | hard-coded in `cliprDemoVideoModelId.ts` | `bytedance/seedance-2.0` | Generates silent Demo remixes from one selected saved Demo clip passed as a reference video. |
...
- Update this file and `docs/operations/security/rate-limits.md`.
