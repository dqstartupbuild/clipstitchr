...
| Clipr non-avatar generated video scenes | `CLIPR_GENERATED_VIDEO_MODEL_ID` | `prunaai/p-video` | Generates vertical 720p visual scenes for B-roll Reel, Text Shot, Voiceover Reel, Product Video, Value Video, Problem/Solution, Objection Handler, How-To, and Soft CTA formats. The app sends no baked-in text, UI, logos, or visible-speaking requests, caps each scene at 20 seconds, copies scene outputs to R2, then composes final Clips with Media Bunny. B-roll Reel remains silent; only Voiceover Reel adds separate avatar-voice audio. |
