...
| CLI AI guide generation | `CLI_DEMO_GUIDE_MODEL_ID` | `openai/gpt-5-mini` | Writes label-only walkthrough guides for `clipstitchr demo guide create` and `clipstitchr demo auto`. This uses the existing provider prediction path but has its own OpenAI text-model default. |
