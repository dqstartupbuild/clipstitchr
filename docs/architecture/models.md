...
| Clipr background music | `CLIPR_MUSIC_MODEL_ID` | `stability-ai/stable-audio-2.5` | Generates a 60 second instrumental music bed when the user opts in. The MP3 is copied to R2 as a separate editable asset and mixed into the video only during export/download. |
