...
| `stitch_scheduled` | Stitch schedule request succeeds in the browser | Tracks stitch ID, Zernio post ID, and platform count. |
...
| `social_publishing_post_scheduled` | Zernio schedule API succeeds | Tracks source type, source ID, Zernio post ID, platform count, queue mode, media size, and audio flag. |
| `social_publishing_analytics_refreshed` | Zernio analytics sync succeeds | No user-entered content is sent. |
