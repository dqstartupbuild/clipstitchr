...
| `stitch_scheduled` | Legacy Post Bridge Stitch schedule request succeeds in the browser | Tracks stitch ID, Post Bridge post ID, and platform count. The in-house durable post path does not reuse this provider-specific event. |
...
| `post_bridge_post_scheduled` | Legacy Post Bridge schedule API succeeds | Tracks source type, source ID, Post Bridge post ID, platform count, queue mode, media size, and audio flag. |
| `post_bridge_analytics_synced` | Legacy Post Bridge analytics sync succeeds | No user-entered content is sent. Read-only migration views never emit it. |
| `hook_lab_post_created` | A public social post is durably saved for analysis | Tracks the platform enum only. |
| `hook_lab_post_analysis_started` | An initial or retry analysis job is accepted | Tracks retry and platform metadata only. |
...
post, provider-job, or prediction IDs.
