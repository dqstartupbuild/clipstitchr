...
- `docs/backend/durable-workflows.md` says provider work must not depend on the
  browser staying open.
- `docs/backend/provider-automation-workflows.md` calls for provider tasks,
  finalizers, retries, queueing, and future autopilot runs.
- `docs/backend/rate-limits.md` already gates expensive surfaces before
  provider calls.
...
- Shared music output copy to shared and owner R2 prefixes.
...
7. Update `docs/backend/rate-limits.md` when the implementation changes
   enforcement points, active job caps, retry behavior, or environment
   variables.

## Current Repo Note

`docs/backend/server-side-media-processing.md` describes `mediaJobs` and a media
