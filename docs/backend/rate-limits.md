...
| CLI demo agent AI planning | `POST /api/cli/demo-agent/plan` | 120/hour/user, burst 20; global 3,000/hour, burst 500 across 5 shards; shared provider bucket 10,000 units/hour, burst 2,000. The route verifies the CLI bearer token, consumes quota, then calls the configured planner model through `CLI_DEMO_AGENT_PLANNER_MODEL_ID`, defaulting to `openai/gpt-5-mini`, to propose one JSON browser-action DSL item from simplified page observation, current guide step, and guide goal context only. The server parser rejects unsupported action types and CSS selectors, and the CLI policy validator still decides whether the proposed action can run. If the planner provider fails or repeats an already-attempted action during a CLI run, the CLI switches to the deterministic local planner for the rest of that run. |
