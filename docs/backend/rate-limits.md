...
| Automatic Stitchr generation | Worker-only automation planner before provider work and the Stitchr Batch tab's signed-in user trigger; provider worker generates Stitchr overlay text plus caption/hashtag copy and creates the media worker job | 10 Stitchr outputs/day/user; global 1,000/day |
