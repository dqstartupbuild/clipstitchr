...
| CLI demo agent AI planning | `POST /api/cli/demo-agent/plan` | 120/hour/user, burst 20; global 3,000/hour, burst 500 across 5 shards; shared provider bucket 10,000 units/hour, burst 2,000. The route verifies the CLI bearer token, consumes quota, then calls the configured writing model through `TEXT_WRITING_MODEL_ID` to propose one JSON browser-action DSL item from simplified page observation only. The server parser rejects unsupported action types and CSS selectors, and the CLI policy validator still decides whether the proposed action can run. |
