...
| Hook Lab Idea analysis | `POST /api/hook-lab/ideas` and `POST /api/hook-lab/ideas/{id}/retry` | Idea creation first consumes `convexRecordSave`. Before a new provider job, the route consumes 30 analyses/day/user with burst 5, 1,000/day globally with burst 200 across 5 shards, and the shared provider-spend guard. Social sources additionally consume 15 imports/day/user with burst 3 and 300/day globally with burst 60 across 5 shards before Apify run creation. A provider-job idempotency hit reports `alreadyReserved` and does not reserve the same work twice. HTTP rejection returns `429` with `Retry-After`, and an already-created Idea moves to `needs_attention` so it can be retried instead of remaining stuck as analyzing. |
