...
| Swipr photo R2 download signed URL | `POST /api/swipr/backgrounds/download-url` | Uses the R2 download signed URL limit after validating the private owner-owned Swipr photo or global Pexels pack photo exists |
...
| Swipr Pexels pack edits | `swiprBackgrounds.addLibraryPackToAccount`, `swiprBackgrounds.removeLibraryPackFromAccount`, `swiprBackgrounds.renameLibraryPack`, `swiprBackgrounds.removeFromLibraryPack`, `swiprBackgrounds.removeLibraryPack` | Pack add/remove account actions consume `convexMetadataUpdate` after validating the global pack or account row. Pack rename and photo removal consume `convexMetadataUpdate` after owner checks and before patching matching owned imported records. Pack delete uses the rate-limited R2 delete route for matching owned imported image objects, then consumes `convexRecordDelete` for the matching record count before deleting Convex records and matching account-pack rows. |
