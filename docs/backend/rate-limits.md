...
- `SHORT_FORM_TEXT_MODEL_ID` defaults to `anthropic/claude-4.5-haiku` for
  Clipr hook/script generation, Stitchr auto-text, and Swipr auto-text.
  Supported workflows include `anthropic/claude-4.5-haiku` and
  `openai/gpt-4.1`. `CLIPR_HOOK_MODEL_ID` remains a backward-compatible
  fallback when `SHORT_FORM_TEXT_MODEL_ID` is not set.
