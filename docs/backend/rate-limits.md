...
| Post Bridge schedule create | `POST /api/post-bridge/schedule` from saved Stitch and Swipe cards | 10 schedules/hour/user, burst 5; 30 schedules/day/user; 2 GB uploaded schedule media/day/user; global 1,000 schedules/day, burst 200 across 5 shards; global 50 GB uploaded schedule media/day across 10 shards. The route validates ownership of the source stitch or swipe, resolves the source product and product-linked account IDs, rejects unsupported media, mixed image/video uploads, multiple-video uploads, image uploads for Stitches, image uploads to YouTube Shorts, and files above `POST_BRIDGE_MAX_MEDIA_BYTES`, consumes quota using total uploaded media bytes, decrypts the user's saved Post Bridge key, verifies selected accounts belong to that key, uploads rendered Swipe PNG carousels or MP4 videos to Post Bridge, creates the scheduled post, then records the returned Post Bridge post ID on the source asset. Swipe audio is mixed into the browser-rendered MP4 before this route is called when sound is used. |
