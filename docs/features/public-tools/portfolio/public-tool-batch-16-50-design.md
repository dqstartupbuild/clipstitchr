...
`docs/features/public-tools/portfolio/public-tool-lead-capture-strategy.md` supersedes those decisions
...
boundary recorded in `docs/operations/email/integration.md`. Until that
...
| 42 | Five-Day App Content Sprint: `/tools/five-day-app-content-sprint` | Guided sprint | Five daily workspaces open after exact-course confirmation, one per 24 hours, with app-owned progress and one Markdown plan. | No finished media or scheduled posts. |
| 43 | UGC-to-App-Ad Mini-Course: `/tools/ugc-to-app-ad-mini-course` | Self-paced course | Five confirmed lessons open one per 24 hours, each with an explanation, example, exercise, rationale, completion check, and app-owned progress. | No creator sourcing, footage analysis, assembly, or export. |
| 44 | App Creative Testing System Workshop: `/tools/app-creative-testing-system-workshop` | Interactive workshop | Explicit workshop confirmation opens a 45-minute recurring-system charter for goals, variable hierarchy, roles, naming, evidence, review cadence, asset flow, and app-owned progress. | No tests, performance ingestion, asset management, or creatives. |
...
| 46 | App Ad Teardown Library — `/tools/app-ad-teardown-library` | Filterable collection | At least twelve original educational pattern teardowns with category, stage, hook, opening, handoff, proof, pacing, CTA, reusable pattern, limitations, and source context. | No copied media, unsupported performance claims, or production. The later hybrid lead-capture layer adds only a text-only Markdown library export. |
...
The later approved lead-capture layer does not change any complete on-page
result in this table. It adds format-matched browser portability for the seven
resources completed in the same rollout: CSV for capabilities 2, 14, 17, and
19, and browser print actions for capabilities 4, 10, and 16. These actions
remain behind the fail-closed `hybrid-v1` rollout described in
`docs/features/public-tools/portfolio/public-tool-lead-capture-strategy.md`; control behavior remains
unchanged.
