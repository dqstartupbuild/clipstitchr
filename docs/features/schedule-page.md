...
- `Scheduled content` loads the signed-in user's Post Bridge posts through
  ...
  scheduled rows can show account names when possible. Queue-scheduled rows
...
authenticated, resolve the user's encrypted Post Bridge key server-side, and
consume the Post Bridge read rate limit before calling Post Bridge.
