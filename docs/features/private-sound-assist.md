...
4. TikTok lookup calls Apify's `clockworks/tiktok-scraper`, extracts sound
   ...
   returns short candidate rows. ClipStitchr does not currently auto-pick a
   trending sound without a user search or TikTok link.
...
6. The last selected sound is stored as editable music metadata on the Clip or
