...
Cropping does not change the original uploaded file. It saves crop metadata and
uses that metadata the next time ClipStitchr renders a poster, Stitch preview,
or export.
...
- `web/lib/clipstitchr/hooks/useClipLibraryState.ts` saves crop updates,
  regenerates affected stitch posters, and clears stale saved stitch renders.
...
Stitch crop changes, the browser regenerates one poster and uploads it through
the existing R2 signed upload path. The old saved Stitch render is cleared so
the next preview or download creates a fresh render with the saved crop.
