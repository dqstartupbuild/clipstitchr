...
export const ContinueIntegration: FC<{
  ...
  searchParams: any;
  logged: boolean;
  publishingBasePath?: string;
}> = (props) => {
  const {
    provider,
    searchParams,
    logged,
    publishingBasePath = '/dashboard/publishing',
  } = props;
  ...
  const modifiedParams = useMemo(() => searchParams, [searchParams]);

  useEffect(() => {
    (async () => {
      if (!supportedProviders.has(provider)) {
        setErrorMessage('This publishing provider is not available.');
        setError(true);
        return;
      }
      ...
      if (data.status === HttpStatusCode.PreconditionFailed) {
        ...
        navigateOrShow(
          `${publishingBasePath}/integrations?precondition=true`,
          returnURL,
          'Precondition failed'
        );
        return;
      ...
      if (data.status === HttpStatusCode.NotAcceptable) {
        ...
        navigateOrShow(
          `${publishingBasePath}/integrations?msg=${msg}`,
          returnURL,
          msg
        );
        return;
      ...
      navigateOrShow(
        `${publishingBasePath}/integrations?added=${provider}&msg=Channel Updated${
  ...
  const onSave = useCallback(
    async (data: any) => {
      ...
      try {
        ...
        navigateOrShow(
          `${publishingBasePath}/integrations?added=${provider}&msg=Channel Added${
    ...
    return (
      <div className="flex flex-1 items-center justify-center text-white relative overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          ...
          {logged && (
            <Redirect
              url={`${publishingBasePath}/integrations`}
              delay={3000}
            />
          )}
        </div>
