...
const allowedIntegrations = ['instagram', 'instagram-standalone', 'tiktok'];
export const PlatformAnalytics = () => {
  ...
  const options = useMemo(() => {
    ...
    const arr = [];
    if (allowedIntegrations.includes(currentIntegration.identifier)) {
      arr.push({
        key: 7,
  ...
  if (!sortedIntegrations.length && !isLoading) {
    return (
      <div className="bg-newBgColorInner p-[20px] flex flex-col gap-[15px] transition-all flex-1 justify-center items-center text-center">
        ...
        <Button
          onClick={() => router.push('/dashboard/publishing/integrations')}
        >
          {t(
