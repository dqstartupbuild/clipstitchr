...
export const readEphemeralPostgresTestUrl = (
  ...
  if (environment["PUBLISHING_TEST_POSTGRES_EPHEMERAL"] !== "true") {
    throw new Error("Ephemeral PostgreSQL test acknowledgement is required.");
  }

  const rawUrl = environment["PUBLISHING_TEST_DATABASE_URL"];

  if (rawUrl === undefined) {
    throw new Error("PUBLISHING_TEST_DATABASE_URL is required.");
  }
