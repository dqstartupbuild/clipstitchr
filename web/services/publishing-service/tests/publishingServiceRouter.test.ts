...
describe("publishing service router", () => {
  ...
  it("fails closed before authentication, rate limits, or route work when Studio Beta is off", async () => {
    const consume = vi.fn(async (request) =>
      createRateLimitDecision(request.action, true, 0),
    );
    const handle = await withServer(
      ...
      async (origin) => {
        ...
        await expect(readinessResponse.json()).resolves.toEqual({
          ...
          status: "not_ready",
          checks: [{ name: "studio-beta", status: "not_ready" }],
        });

        const requestId = "request-disabled-123";
        const response = await fetch(`${origin}/v1/posts`, {
          method: "POST",
          headers: {
            Authorization: `Bearer ${issueAssertion(requestId)}`,
          ...
          body: JSON.stringify({ caption: "Must not run" }),
        });
