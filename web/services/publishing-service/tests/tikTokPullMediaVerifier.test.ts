...
describe("createTikTokPullMediaVerifier", () => {
  it("requires the exact configured gateway origin and a successful range-capable HEAD", async () => {
    ...
    await expect(
      verify(
        new URL(
          `https://media.clipstitchr.invalid/api/studio/publishing/media/${token}`,
        ),
      ),
    ).resolves.toBe(true);
  ...
  it("does no network work for foreign origins, query strings, or nested paths", async () => {
    ...
    await expect(
      verify(new URL(`https://evil.invalid/api/studio/publishing/media/${token}`)),
    ).resolves.toBe(false);
    await expect(
      verify(
        new URL(
          `https://media.clipstitchr.invalid/api/studio/publishing/media/${token}?copy=1`,
        ),
      ),
    ).resolves.toBe(false);
    await expect(
      verify(
        new URL(
          `https://media.clipstitchr.invalid/api/studio/publishing/media/${token}/extra`,
        ),
      ),
    ).resolves.toBe(false);
