...
beforeAll(async () => {
  ...
  const youtubeProductScopeMigrationSql = await readPublishingSqlFile(
    "../../prisma/migrations/20260812140000_add_youtube_product_scope/migration.sql",
  );
  ...
  await administrator.query(youtubeProductScopeMigrationSql);
...
describe.sequential("publishing PostgreSQL persistence", () => {
  it("creates immutable Clerk tenant mappings and isolates integrations and media", async () => {
    ...
    const integrationYouTube = await createPublishingIntegration(database, {
      tenantKey: tenantKeyA,
      internalId: "youtube-alpha",
      name: "Alpha YouTube",
      provider: "youtube",
    });
    integrationYouTubeId = integrationYouTube.id;
    ...
    ).toEqual(expect.arrayContaining([integrationAId, integrationYouTubeId]));
    expect(
  ...
  it("persists only context-bound encrypted provider tokens", async () => {
    ...
    const youtubeRefreshToken = "youtube-refresh-token-that-must-not-leak";
    const youtubeRefreshSecret = await storePublishingIntegrationSecret(
      database,
      {
        tenantKey: tenantKeyA,
        integrationId: integrationYouTubeId,
        provider: "youtube",
        tokenKind: "refresh",
        plaintextToken: youtubeRefreshToken,
        cipherKey,
      },
    );
    expect(youtubeRefreshSecret.envelope).not.toContain(youtubeRefreshToken);
    await expect(
      readPublishingIntegrationSecret(database, {
        tenantKey: tenantKeyA,
        integrationId: integrationYouTubeId,
    ...
    ).resolves.toBe(youtubeRefreshToken);
  });
  ...
  it("commits one post, state, attempt, and outbox under concurrent idempotent requests", async () => {
    ...
    const anotherProduct = await createPublishingDestination(
      database,
      {
        ...input,
        productId: "product_alpha_second",
        intent: { kind: "draft" },
      },
      new Date("2026-08-02T12:00:00.000Z"),
    );
    expect(anotherProduct.created).toBe(true);
    expect(anotherProduct.workflowId).not.toBe(first.workflowId);
    await database.clipPublishingPostState.delete({
      where: { id: anotherProduct.postStateId },
    });
    await database.post.delete({ where: { id: anotherProduct.postId } });

    const destinationB = await createPublishingDestination(
      database,
      {
        tenantKey: tenantKeyB,
        productId: "product_bravo",
