...
let youtubeProductScopeMigrationSql = "";
let focusedCoreSql = "";
...
beforeAll(async () => {
  ...
  youtubeProductScopeMigrationSql = await readPublishingSqlFile(
...
describe.sequential("publishing PostgreSQL migrations", () => {
  ...
  it("adds YouTube, Studio output kinds, and a nullable-safe Product column", async () => {
    await administrator.query(`SET search_path TO "${freshSchema}"`);
    const enumLabels = await administrator.query<{
      enumlabel: string;
      typname: string;
    }>(
    ...
    const productColumn = await administrator.query<{ is_nullable: string }>(
      `SELECT is_nullable
       FROM information_schema.columns
       WHERE table_schema = $1
         AND table_name = 'ClipPublishingPostState'
    ...
    expect(enumLabels.rows).toEqual(
      expect.arrayContaining([
        { typname: "ClipPublishingProvider", enumlabel: "YOUTUBE" },
        {
          typname: "ClipPublishingProviderOperation",
          enumlabel: "YOUTUBE_RESUMABLE_UPLOAD",
        },
        {
          typname: "ClipPublishingSourceKind",
          enumlabel: "STUDIO_CLIP_OUTPUT",
        },
        {
          typname: "ClipPublishingSourceKind",
          enumlabel: "STUDIO_STITCH_OUTPUT",
        },
      ]),
    );
    expect(productColumn.rows).toEqual([{ is_nullable: "YES" }]);
  });
});
