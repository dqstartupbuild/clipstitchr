...
import { mapPublishingSourceKind } from "../src/persistence/mapPublishingSourceKind.js";
import { mapPublishingProvider } from "../src/persistence/mapPublishingProvider.js";
...
describe("publishing persistence validation", () => {
  ...
  it("persists YouTube and both durable Studio output source kinds", () => {
    expect(mapPublishingProvider("youtube")).toBe("YOUTUBE");
    expect(mapPublishingSourceKind("studio-clip-output")).toBe(
      "STUDIO_CLIP_OUTPUT",
    );
    expect(mapPublishingSourceKind("studio-stitch-output")).toBe(
      "STUDIO_STITCH_OUTPUT",
    );
  });

  it("binds canonical idempotency inputs to Product scope", () => {
    const base = {
      schemaVersion: 2,
    ...
    expect(
      createCanonicalPublishingRequestHash({ ...base, productId: "product_1" }),
    ).not.toBe(
      createCanonicalPublishingRequestHash({ ...base, productId: "product_2" }),
    );
  });
