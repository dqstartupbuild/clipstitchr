  ...
  return JSON.stringify(
    {
      ...
      rules: [
        "Use only action types: navigate, click, type, uploadFile, waitFor, scroll, selectOption, pressKey, clearField, scrollToText, scrollToControl, clickFirstMatching, clickCardAction, selectCard, waitForJob, waitForElementEnabled, chooseFileFromLibrary, toggle, setMode, openMenu, chooseMenuItem, closeDialog, dragAndDrop, setSlider, playPauseMedia, seekMedia, downloadFile, copyToClipboard, screenshot, finishStep, stop.",
        "Use only allowedClickRoles for click.target.role, or omit role and use label/name/text.",
        "For route-opening steps like Open /dashboard/tool, return navigate with that exact local path.",
        "For typing steps, return type with target.label and valueText in the same action.",
        "Use semantic action shapes for card selections, selects, toggles, menus, modes, sliders, downloads, copy buttons, media controls, and drag/drop.",
