...
export function createProductEnrichmentPrompt({
  ...
  return [
    ...
    "When Website details are supplied, return an improved productDetails truth summary even when the user already entered product details. Preserve the user's positioning, add only facts supported by the website, and include the product type, platform, intended user, concrete current features or workflow, meaningful limits, and supported outcome. Make the summary useful for writing truthful ads, not just a slogan.",
    "When Website details are not supplied, infer productDetails only if Product details is blank. If Product details is already present and there is no Website details block, return an empty string for productDetails.",
    ...
    "Never replace a user-provided claim with an unsupported website inference. Website-backed productDetails may expand a vague user summary, while audienceDetails remains a blank-field prefill only.",
