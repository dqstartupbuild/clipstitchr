...
export function createStitchrHookGenerationPrompt({
  ...
  const compositionLines = getStitchrTextCompositionPromptLines(
    stitchrClipContexts,
  );

  return [
    "Write three native UGC discovery overlays and one feed caption for a vertical stitched video.",
    "Write for the supplied ordered source roles. There is no generated voiceover.",
    ...compositionLines,
    "The viewer should feel close to what is visibly happening, not like a brand is advertising a feature.",
    ...
    "",
    "Rules:",
    "- Use the Hook Library candidates only as creative mechanisms. Select only candidate IDs from the supplied list, then write new source-grounded copy. Never paste a template with its placeholders merely filled.",
    "- Return the strongest three distinct options for visual fit, sound-off clarity, specificity, and product truth. Never print the drafts or scoring process.",
    ...
    "- The three options must use meaningfully different creator angles when a creator is visible, chosen from Self-callout, Reluctant discovery, Expectation reversal, Excuse removed, Identity moment, Discovery question, or visible payoff.",
    "- Make the overlay feel native to the visible source. Do not merely describe the visible action, repeat a clip title, paraphrase Product details, or paste an existing AI hook hint.",
    ...
    "- Prefer first-person, me, POV, conversational question, reluctant-admission, or self-callout framing only when it fits the visible source.",
    "- Prefer not to name the product in the overlay unless the name is essential to understanding the visible source.",
    ...
    "- Only keep options that the visible ordered source(s) support without voiceover or caption context.",
