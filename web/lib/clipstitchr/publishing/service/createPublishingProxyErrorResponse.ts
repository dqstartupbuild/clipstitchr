...
export function createPublishingProxyErrorResponse(error: unknown): Response {
  ...
  } else if (error instanceof StudioBetaApiAccessError) {
    status = error.status;
    code = status === 401 ? "authentication_required" : "studio_access_required";
    message =
      status === 401
        ? "Sign in to continue."
        : "Studio Beta access is required for Postiz Beta.";
