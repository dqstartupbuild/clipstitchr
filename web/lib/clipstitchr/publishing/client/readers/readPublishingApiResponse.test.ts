...
describe("readPublishingApiResponse", () => {
  ...
  it("requires Product identity on every post row and accepts YouTube", async () => {
    const post = {
      ...
      productId: "product_1",
      provider: "youtube",
      resultUrl: "https://www.youtube.com/watch?v=video_1",
      scheduledAt: null,
      status: "published",
      statusMessage: null,
      timeZone: "America/Detroit",
    ...
    await expect(
      readPublishingApiResponse(
        new Response(JSON.stringify({ posts: [post] })),
        publishingApiSchemas.postsResponse,
      ),
    ).resolves.toEqual({ posts: [post] });
    const missingProduct: Partial<typeof post> = { ...post };
    delete missingProduct.productId;
    await expect(
      readPublishingApiResponse(
        new Response(JSON.stringify({ posts: [missingProduct] })),
        publishingApiSchemas.postsResponse,
      ),
    ).rejects.toMatchObject({ code: "invalid_response" });
  });
