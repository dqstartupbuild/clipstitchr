...
const provider = z.enum(["instagram", "tiktok", "youtube"]);
const status = z.enum([
  "action-required",
  "canceled",
  "draft",
...
const postSummary = z
  ...
  .strict()
  .superRefine(({ provider: resultProvider, resultUrl }, context) => {
    if (!isPublishingProviderResultUrl(resultProvider, resultUrl)) {
      context.addIssue({
        code: "custom",
        message: "Provider result URL does not match its provider.",
        path: ["resultUrl"],
      });
    }
  });
...
export const publishingApiSchemas = Object.freeze({
  ...
  integrationsResponse: z
    .object({ providers: z.array(providerGroup).length(3) })
    .strict()
    .superRefine(({ providers }, context) => {
      ...
      if (
        ...
        context.addIssue({
          code: "custom",
          message: "Every supported provider must be represented.",
        });
      }
