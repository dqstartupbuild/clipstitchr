...
export function resolveOwnedPublishingMediaSource({
  ...
  if (
    (descriptor.kind === "stitch" ||
      descriptor.kind === "studio-clip-output" ||
      descriptor.kind === "studio-stitch-output") &&
  ...
  if (
    ...
    (mediaObjects.length !== 1 ||
      (!mediaObjects[0].contentType.startsWith("video/") &&
        !mediaObjects[0].contentType.startsWith("image/")))
  ) {
    throw new PublishingMediaValidationError(
      "invalid_metadata",
      "This saved Library source must resolve to one durable photo or video object.",
    );
  }
