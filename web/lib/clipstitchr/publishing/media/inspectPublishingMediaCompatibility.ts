...
export function inspectPublishingMediaCompatibility(
  ...
  if (mediaObjects.length === 0) {
    issues.push({
      code: "media_required",
      message: `${provider === "instagram" ? "Instagram" : "TikTok"} needs at least one media item.`,
      severity: "error",
    });
  }
  ...
  mediaObjects.forEach((mediaObject, mediaIndex) => {
    ...
    if (!providerMimeTypes.has(contentType)) {
      issues.push({
        ...
        mediaIndex,
        message: `${provider === "instagram" ? "Instagram" : "TikTok"} does not commonly accept ${contentType} through this publishing path.`,
        severity: "error",
      });
    }
