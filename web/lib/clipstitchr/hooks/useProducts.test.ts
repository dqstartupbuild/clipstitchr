...
function createProductDocument(overrides: Record<string, unknown> = {}) {
  return {
...
describe("useProducts", () => {
  beforeEach(() => {
    ...
    mocks.useMutation.mockReturnValue(mocks.removeProductMutation);
    mocks.useQuery.mockReturnValue([createProductDocument()]);
  ...
  it("creates, updates, and deletes products with saving state transitions", async () => {
    const state = useProducts();
    const input = {
      audienceDetails: "Founders",
      name: "Launch Kit",
