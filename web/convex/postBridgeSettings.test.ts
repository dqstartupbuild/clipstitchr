...
import { clearSecret, saveSecret } from "./postBridgeSettings";
...
const mocks = vi.hoisted(() => ({
  clearPostBridgeSocialAccountIdsForOwner: vi.fn(),
...
vi.mock("./clearPostBridgeSocialAccountIdsForOwner", () => ({
  clearPostBridgeSocialAccountIdsForOwner:
    mocks.clearPostBridgeSocialAccountIdsForOwner,
}));
...
describe("postBridgeSettings", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.clearPostBridgeSocialAccountIdsForOwner.mockResolvedValue(undefined);
  ...
  it("clears product account defaults before deleting saved settings", async () => {
    const settings = { _id: "post_bridge_settings_doc" };
    ...
    expect(mocks.clearPostBridgeSocialAccountIdsForOwner).toHaveBeenCalledWith(
      ctx,
      "owner_123",
      expect.any(String),
    );
    expect(ctx.db.delete).toHaveBeenCalledWith("post_bridge_settings_doc");
  });

  it("clears product account defaults when a saved key changes", async () => {
    const settings = {
      _id: "post_bridge_settings_doc",
      apiKeyLast4: "1111",
    };
    const queryChain = createQueryChain(settings);
    const ctx = {
    ...
    ).resolves.toBe("post_bridge_settings_doc");
    expect(mocks.clearPostBridgeSocialAccountIdsForOwner).toHaveBeenCalledWith(
      ctx,
      "owner_123",
      "2026-07-03T12:00:00.000Z",
    );
    expect(ctx.db.patch).toHaveBeenCalledWith("post_bridge_settings_doc", {
      apiKeyLast4: "2222",
      encryptedApiKey: "encrypted_new_key",
      lastVerifiedAt: "2026-07-03T12:00:00.000Z",
      updatedAt: "2026-07-03T12:00:00.000Z",
