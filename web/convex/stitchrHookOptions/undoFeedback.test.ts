...
import { undoFeedback } from "./undoFeedback";
...
function getHandler() {
  return (undoFeedback as unknown as ConvexFunction).handler;
}

describe("stitchrHookOptions.undoFeedback", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("resets only the chosen option and removes its normalized product memory", async () => {
    const option = {
      _id: "option_doc_1",
    ...
    expect(ctx.db.patch).toHaveBeenCalledWith("option_doc_1", {
    ...
    expect(ctx.db.patch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        rejectedHookExamples: ["Keep this rejection"],
      }),
    );
