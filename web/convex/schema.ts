...
import { studioBetaAuditEventTypeValidator } from "./validators/studioBetaAuditEventType";
import { studioBetaGrantStatusValidator } from "./validators/studioBetaGrantStatus";
import { studioLazyReelBriefApprovalStateValidator } from "./validators/studioLazyReelBriefApprovalState";
import { studioLazyReelBriefHandoffDestinationValidator } from "./validators/studioLazyReelBriefHandoffDestination";
import { studioLazyReelBriefStatusValidator } from "./validators/studioLazyReelBriefStatus";
import { studioLazyReelIdentityValidator } from "./validators/studioLazyReelIdentity";
import { studioLazyReelJsonSnapshotValidator } from "./validators/studioLazyReelJsonSnapshot";
import { studioLazyReelReportStatusValidator } from "./validators/studioLazyReelReportStatus";
import { studioLazyReelRunFailureValidator } from "./validators/studioLazyReelRunFailure";
import { studioLazyReelRunOutcomeValidator } from "./validators/studioLazyReelRunOutcome";
import { studioLazyReelRunStatusValidator } from "./validators/studioLazyReelRunStatus";
import { studioEditorProjectStatusValidator } from "./validators/studioEditorProjectStatus";
import { studioEditorWriteOperationValidator } from "./validators/studioEditorWriteOperation";
...
import { studioClipsExecutionAvailabilityValidator } from "./validators/studioClipsExecutionAvailability";
...
import { studioClipsPlatformPresetValidator } from "./validators/studioClipsPlatformPreset";
import { studioClipsRenderOperationKindValidator } from "./validators/studioClipsRenderOperationKind";
...
import { studioClipsTaskStatusValidator } from "./validators/studioClipsTaskStatus";
import { studioClipsWorkerStatusValidator } from "./validators/studioClipsWorkerStatus";
import { studioClipsWriteOperationValidator } from "./validators/studioClipsWriteOperation";
import { studioReelHandoffMetadataValidator } from "./validators/studioReelHandoffMetadata";
import { studioReelOutputStatusValidator } from "./validators/studioReelOutputStatus";
...
import { studioReelReviewStatusValidator } from "./validators/studioReelReviewStatus";
import { studioReelRunKindValidator } from "./validators/studioReelRunKind";
...
export default defineSchema({
  studioClipsTasks: defineTable({
    ownerId: v.string(),
    id: v.string(),
    productId: v.string(),
    status: studioClipsTaskStatusValidator,
    source: studioClipsSourceValidator,
    options: studioClipsTaskOptionsValidator,
    execution: studioClipsExecutionAvailabilityValidator,
    recordVersion: v.number(),
    revision: v.number(),
    inputByteLength: v.number(),
    attempt: v.number(),
    progressPercent: v.number(),
    checkpoint: v.optional(studioClipsCheckpointValidator),
    latestCode: v.optional(studioClipsProgressCodeValidator),
    resumeCheckpoint: v.optional(studioClipsResumeCheckpointValidator),
    resumeRevision: v.optional(v.number()),
    failure: v.optional(studioClipsFailureValidator),
    analysisSnapshotVersion: v.optional(v.number()),
    analysisSnapshotJson: v.optional(v.string()),
    analysisSnapshotByteLength: v.optional(v.number()),
    ...
    .index("by_owner_product_id", ["ownerId", "productId", "id"])
    .index("by_owner_product_created", ["ownerId", "productId", "createdAt"])
    .index("by_owner_product_status_created", [
      "ownerId",
      "productId",
      "status",
      "createdAt",
  ...
  studioClipsTaskEvents: defineTable({
    ownerId: v.string(),
    productId: v.string(),
    taskId: v.string(),
    attempt: v.number(),
  ...
  }).index("by_owner_product_task_occurred", [
    "ownerId",
    "productId",
    "taskId",
    "occurredAt",
  ]),
  studioClipsCheckpoints: defineTable({
    ownerId: v.string(),
    productId: v.string(),
    taskId: v.string(),
    attempt: v.number(),
    ...
    snapshotByteLength: v.number(),
    createdAt: v.string(),
  })
    .index("by_task_revision", ["taskId", "revision"])
    .index("by_owner_product_task_revision", [
      "ownerId",
      "productId",
      "taskId",
      "revision",
    ]),
  studioClipsOutputs: defineTable({
    ownerId: v.string(),
    id: v.string(),
    productId: v.string(),
    taskId: v.string(),
    ...
    captionCuesJson: v.optional(v.string()),
    captionCuesByteLength: v.optional(v.number()),
    captionsBurned: v.optional(v.boolean()),
    cleanMasterContentType: v.optional(v.string()),
    cleanMasterObjectKey: v.optional(v.string()),
    cleanMasterSha256: v.optional(v.string()),
    cleanMasterSizeBytes: v.optional(v.number()),
    durationSeconds: v.optional(v.number()),
    fileName: v.optional(v.string()),
    hasAudio: v.optional(v.boolean()),
    height: v.optional(v.number()),
    libraryClipId: v.optional(v.string()),
    parentOutputId: v.optional(v.string()),
    platformPreset: v.optional(studioClipsPlatformPresetValidator),
    renderRevisionId: v.optional(v.string()),
    objectKey: v.string(),
    sha256: v.string(),
    sizeBytes: v.number(),
    ...
    recordVersion: v.number(),
    revision: v.number(),
    editSnapshotVersion: v.number(),
    editSnapshotJson: v.string(),
    editSnapshotByteLength: v.number(),
    createdAt: v.string(),
    updatedAt: v.string(),
  })
    .index("by_owner_product_id", ["ownerId", "productId", "id"])
    .index("by_owner_product_task_created", [
      "ownerId",
      "productId",
      "taskId",
      "createdAt",
    ])
    .index("by_owner_product_render_revision_created", [
      "ownerId",
      "productId",
      "renderRevisionId",
      "createdAt",
  ...
  studioClipsWriteReceipts: defineTable({
    ownerId: v.string(),
    productId: v.string(),
    idempotencyKey: v.string(),
    operation: studioClipsWriteOperationValidator,
    targetId: v.string(),
  ...
  studioClipsCostReservations: defineTable({
    ownerId: v.string(),
    productId: v.string(),
    taskId: v.string(),
    attempt: v.number(),
    ...
    createdAt: v.string(),
  }).index("by_task_attempt_stage", ["taskId", "attempt", "stage"]),
  studioClipsRenderRevisions: defineTable({
    ownerId: v.string(),
    id: v.string(),
    productId: v.string(),
    taskId: v.string(),
    ...
    status: studioClipsTaskStatusValidator,
    execution: studioClipsExecutionAvailabilityValidator,
    operationKind: studioClipsRenderOperationKindValidator,
    operationJson: v.string(),
    operationByteLength: v.number(),
    sourceOutputsJson: v.string(),
    sourceOutputsByteLength: v.number(),
    platformPreset: v.optional(studioClipsPlatformPresetValidator),
    ...
    .index("by_owner_product_id", ["ownerId", "productId", "id"])
    .index("by_owner_product_created", ["ownerId", "productId", "createdAt"])
    .index("by_owner_product_status_created", ["ownerId", "productId", "status", "createdAt"])
    .index("by_status_created", ["status", "createdAt"])
    .index("by_status_lease_expiry", ["status", "leaseExpiresAt"]),
  studioClipsProductStyles: defineTable({
    ownerId: v.string(),
    productId: v.string(),
    revision: v.number(),
    lastRenderRevisionId: v.optional(v.string()),
    recordVersion: v.number(),
    style: studioClipsCaptionStyleValidator,
    createdAt: v.string(),
    updatedAt: v.string(),
  }).index("by_owner_product", ["ownerId", "productId"]),
  studioReelRecipes: defineTable({
    ownerId: v.string(),
    id: v.string(),
    productId: v.string(),
    pipeline: studioReelPipelineValidator,
    status: studioReelRecipeStatusValidator,
    recordVersion: v.number(),
    revision: v.number(),
    recipeVersion: v.number(),
    recipeJson: v.string(),
    recipeByteLength: v.number(),
    createdAt: v.string(),
    updatedAt: v.string(),
    archivedAt: v.optional(v.string()),
  })
    .index("by_owner_product_id", ["ownerId", "productId", "id"])
    .index("by_owner_product_updated", ["ownerId", "productId", "updatedAt"])
    .index("by_owner_product_status_updated", [
      "ownerId",
      "productId",
      "status",
      "updatedAt",
    ]),
  studioReelGenerationRuns: defineTable({
    ownerId: v.string(),
    id: v.string(),
    productId: v.string(),
    kind: studioReelRunKindValidator,
    parentRunId: v.optional(v.string()),
    reviewSubsetId: v.optional(v.string()),
    status: studioReelRunStatusValidator,
    recordVersion: v.number(),
    revision: v.number(),
    recipeIds: v.array(v.string()),
    requestedCount: v.number(),
    providerReadiness: v.array(studioReelProviderReadinessValidator),
    providerIntents: v.array(studioReelProviderIntentValidator),
    idempotencyKey: v.string(),
    attempt: v.number(),
    workerLeaseAttempt: v.optional(v.number()),
    workerLeaseId: v.optional(v.string()),
    workerLeaseWorkerId: v.optional(v.string()),
    workerLeaseExpiresAt: v.optional(v.string()),
    executionProgressPercent: v.optional(v.number()),
    executionCheckpoint: v.optional(studioReelWorkerCheckpointValidator),
    executionRecipeIndex: v.optional(v.number()),
    executionCode: v.optional(studioReelWorkerProgressCodeValidator),
    resumeRevision: v.optional(v.number()),
    cancelRequestedAt: v.optional(v.string()),
    startedAt: v.optional(v.string()),
    createdAt: v.string(),
    ...
    failureKind: v.optional(studioReelWorkerFailureKindValidator),
  })
    .index("by_owner_product_id", ["ownerId", "productId", "id"])
    .index("by_owner_product_parent", ["ownerId", "productId", "parentRunId"])
    .index("by_owner_product_created", ["ownerId", "productId", "createdAt"])
    .index("by_owner_product_status_updated", [
      "ownerId",
      "productId",
    ...
    .index("by_status_created", ["status", "createdAt"])
    .index("by_status_lease_expiry", ["status", "workerLeaseExpiresAt"]),
  studioReelReviewSubsets: defineTable({
    ownerId: v.string(),
    id: v.string(),
    productId: v.string(),
    generationRunId: v.string(),
    status: studioReelReviewStatusValidator,
    recordVersion: v.number(),
    revision: v.number(),
    selectedRecipeIds: v.array(v.string()),
    remainingRecipeIds: v.array(v.string()),
    coverageKeys: v.array(v.string()),
    ...
    .index("by_owner_product_id", ["ownerId", "productId", "id"])
    .index("by_owner_product_run", ["ownerId", "productId", "generationRunId"]),
  studioReelOutputs: defineTable({
    ownerId: v.string(),
    id: v.string(),
    productId: v.string(),
    generationRunId: v.string(),
    recipeId: v.string(),
    status: studioReelOutputStatusValidator,
    recordVersion: v.number(),
    revision: v.number(),
    objectKey: v.string(),
    contentType: v.string(),
    ...
    videoCodec: v.optional(v.string()),
    audioCodec: v.optional(v.string()),
    handoff: v.optional(studioReelHandoffMetadataValidator),
    idempotencyKey: v.string(),
    createdAt: v.string(),
    updatedAt: v.string(),
    ...
    .index("by_owner_product_id", ["ownerId", "productId", "id"])
    .index("by_owner_product_run_created", [
      "ownerId",
      "productId",
      "generationRunId",
      "createdAt",
    ])
    .index("by_owner_product_recipe_created", [
      "ownerId",
      "productId",
      "recipeId",
      "createdAt",
    ]),
  studioReelWriteReceipts: defineTable({
    ownerId: v.string(),
    productId: v.string(),
    idempotencyKey: v.string(),
    operation: studioReelWriteOperationValidator,
    targetId: v.string(),
    requestFingerprint: v.string(),
    createdAt: v.string(),
  }).index("by_owner_idempotency", ["ownerId", "idempotencyKey"]),
  studioReelWorkerEvents: defineTable({
    ownerId: v.string(),
    productId: v.string(),
    runId: v.string(),
    runAttempt: v.number(),
    ...
    progressPercent: v.number(),
    occurredAt: v.string(),
  }).index("by_owner_product_run_occurred", [
    "ownerId",
    "productId",
    "runId",
    "occurredAt",
  ]),
  studioReelWorkerCheckpoints: defineTable({
    ownerId: v.string(),
    productId: v.string(),
    runId: v.string(),
    runAttempt: v.number(),
    ...
    snapshotByteLength: v.number(),
    createdAt: v.string(),
  }).index("by_owner_product_run_attempt_revision", [
    "ownerId",
    "productId",
    "runId",
    "runAttempt",
  ...
  studioReelWorkerCostReservations: defineTable({
    ownerId: v.string(),
    productId: v.string(),
    runId: v.string(),
    runAttempt: v.number(),
    ...
    createdAt: v.string(),
  }).index("by_run_attempt_invocation", [
    "runId",
    "runAttempt",
    "invocationId",
  ]),
  studioEditorProjects: defineTable({
    ownerId: v.string(),
    id: v.string(),
    productId: v.string(),
    name: v.string(),
    status: studioEditorProjectStatusValidator,
    recordVersion: v.number(),
    revision: v.number(),
  ...
  studioEditorProjectRevisions: defineTable({
    ownerId: v.string(),
    projectId: v.string(),
    productId: v.string(),
    name: v.string(),
    ...
    snapshotJson: v.string(),
    snapshotByteLength: v.number(),
    operation: studioEditorWriteOperationValidator,
    createdAt: v.string(),
  }).index("by_owner_product_project_revision", [
    "ownerId",
    "productId",
    "projectId",
    "revision",
  ]),
  studioEditorProjectWriteReceipts: defineTable({
    ownerId: v.string(),
    projectId: v.string(),
    productId: v.string(),
    idempotencyKey: v.string(),
    ...
    requestFingerprint: v.string(),
    resultingRevision: v.number(),
    resultingStatus: studioEditorProjectStatusValidator,
    createdAt: v.string(),
  }).index("by_owner_idempotency", ["ownerId", "idempotencyKey"]),
  studioBetaAccessGrants: defineTable({
    ownerId: v.string(),
    status: studioBetaGrantStatusValidator,
    grantedBy: v.string(),
    grantedAt: v.string(),
  ...
  studioBetaPreferences: defineTable({
    ownerId: v.string(),
    enabled: v.boolean(),
    createdAt: v.string(),
    updatedAt: v.string(),
  }).index("by_owner", ["ownerId"]),
  studioBetaAuditEvents: defineTable({
    ownerId: v.string(),
    actorId: v.string(),
    eventType: studioBetaAuditEventTypeValidator,
    reason: v.optional(v.string()),
    createdAt: v.string(),
  })
    .index("by_owner_created", ["ownerId", "createdAt"])
    .index("by_created", ["createdAt"]),
  studioLazyReelResearchRuns: defineTable({
    ownerId: v.string(),
    id: v.string(),
    productId: v.string(),
    identity: studioLazyReelIdentityValidator,
    status: studioLazyReelRunStatusValidator,
    outcome: v.optional(studioLazyReelRunOutcomeValidator),
    recordVersion: v.number(),
    sourceSnapshotVersion: v.string(),
    idempotencyKey: v.string(),
    inputSnapshot: studioLazyReelJsonSnapshotValidator,
    resultSnapshot: v.optional(studioLazyReelJsonSnapshotValidator),
    artifactSummary: v.optional(studioLazyReelJsonSnapshotValidator),
    failure: v.optional(studioLazyReelRunFailureValidator),
    createdAt: v.string(),
    updatedAt: v.string(),
    completedAt: v.optional(v.string()),
    failedAt: v.optional(v.string()),
    ...
    .index("by_owner_product_created", ["ownerId", "productId", "createdAt"])
    .index("by_owner_product_id", ["ownerId", "productId", "id"])
    .index("by_owner_product_idempotency", [
      "ownerId",
      "productId",
      "idempotencyKey",
    ]),
  studioLazyReelSavedReports: defineTable({
    ownerId: v.string(),
    id: v.string(),
    productId: v.string(),
    ...
    title: v.string(),
    identity: studioLazyReelIdentityValidator,
    status: studioLazyReelReportStatusValidator,
    recordVersion: v.number(),
    sourceSnapshotVersion: v.string(),
    reportSnapshot: studioLazyReelJsonSnapshotValidator,
    createdAt: v.string(),
    updatedAt: v.string(),
    archivedAt: v.optional(v.string()),
  })
    .index("by_owner_product_created", ["ownerId", "productId", "createdAt"])
    .index("by_owner_product_id", ["ownerId", "productId", "id"]),
  studioLazyReelCreativeBriefs: defineTable({
    ownerId: v.string(),
    id: v.string(),
    productId: v.string(),
    ...
    title: v.string(),
    identity: studioLazyReelIdentityValidator,
    status: studioLazyReelBriefStatusValidator,
    approvalState: studioLazyReelBriefApprovalStateValidator,
    handoffDestination: v.optional(
      studioLazyReelBriefHandoffDestinationValidator,
    ),
    recordVersion: v.number(),
    sourceSnapshotVersion: v.string(),
    briefSnapshot: studioLazyReelJsonSnapshotValidator,
    createdAt: v.string(),
    updatedAt: v.string(),
    approvalUpdatedAt: v.string(),
    approvedAt: v.optional(v.string()),
    archivedAt: v.optional(v.string()),
  })
    .index("by_owner_product_created", ["ownerId", "productId", "createdAt"])
