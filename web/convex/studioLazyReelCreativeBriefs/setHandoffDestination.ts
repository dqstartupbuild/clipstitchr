...
export const setHandoffDestination = mutation({
  args: {
    id: v.string(),
    productId: v.string(),
    destination: v.optional(studioLazyReelBriefHandoffDestinationValidator),
  },
  handler: async (ctx, args) => {
    ...
    if (
      ...
      throw new Error("Approve the active creative brief before choosing a handoff.");
    }

    if (existing.handoffDestination === args.destination) {
      return existing;
    }
    ...
    const fields = {
      handoffDestination: args.destination,
