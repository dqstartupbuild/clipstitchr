...
export const submitControl = mutation({
  args: {
    clientKey: v.string(),
    email: v.string(),
    name: v.string(),
  ...
  handler: async (ctx, { clientKey, email, name, secret, source }) => {
    ...
    if (!clientKeyPattern.test(clientKey)) throw new Error("Invalid client key.");
    ...
    const existingEntry = await ctx.db
      ...
      .withIndex("by_normalized_email", (query) =>
        query.eq("normalizedEmail", normalizedInput.email),
      )
      .unique();
