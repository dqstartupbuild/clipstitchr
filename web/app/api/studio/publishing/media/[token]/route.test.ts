...
import {
  GET,
  HEAD,
} from "@/app/api/studio/publishing/media/[token]/route";
...
describe("GET and HEAD /api/studio/publishing/media/[token]", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("streams a complete GET directly with the exact R2 identity and no redirect", async () => {
    ...
    const response = await GET(
      new Request(`${origin}/api/studio/publishing/media/${token}`),
      createContext(token),
    );
  ...
  it("rejects foreign origins and expired grants before quota or R2", async () => {
    ...
    const foreignResponse = await GET(
      new Request(`https://foreign.test/api/studio/publishing/media/${token}`),
      createContext(token),
    );
