...
import { DELETE as disconnectIntegration } from "@/app/api/studio/publishing/integrations/[integrationId]/route";
import { POST as connectIntegration } from "@/app/api/studio/publishing/integrations/[integrationId]/connect/route";
import { POST as refreshIntegration } from "@/app/api/studio/publishing/integrations/[integrationId]/refresh/route";
import { GET as listIntegrations } from "@/app/api/studio/publishing/integrations/route";
import { GET as getTikTokCreatorInfo } from "@/app/api/studio/publishing/integrations/tiktok/creator-info/route";
import { GET as completeOAuthCallback } from "@/app/api/studio/publishing/oauth/[provider]/callback/route";
...
describe("publishing integration API routes", () => {
  ...
  it("forwards every public provider and a bounded JSON connect body", async () => {
    ...
    const response = await connectIntegration(
      new Request(
        "https://clipstitchr.test/api/studio/publishing/integrations/instagram/connect",
        {
          method: "POST",
    ...
    expect(mocks.requestPublishingService).toHaveBeenCalledWith({
      ...
      body: { returnPath: "/dashboard/studio/publishing/integrations" },
      method: "POST",
    ...
    const youtube = await connectIntegration(
      new Request(
        "https://clipstitchr.test/api/studio/publishing/integrations/youtube/connect",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: "{}",
    ...
    expect(mocks.requestPublishingService).toHaveBeenLastCalledWith({
      action: "publishing.integrations.connect",
      body: {},
      method: "POST",
      path: "/v1/integrations/youtube/connect",
    });

    const unsupported = await connectIntegration(
      new Request(
        "https://clipstitchr.test/api/studio/publishing/integrations/facebook/connect",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: "{}",
      ...
      { params: Promise.resolve({ integrationId: "facebook" }) },
    );
    expect(unsupported.status).toBe(404);
    expect(mocks.requestPublishingService).toHaveBeenCalledTimes(2);
  });
  ...
  it("forwards refresh, disconnect, and creator-info with distinct actions", async () => {
    ...
    await getTikTokCreatorInfo(
      new Request(
        "https://clipstitchr.test/api/studio/publishing/integrations/tiktok/creator-info?integrationId=integration_123",
      ),
    );
  ...
  it("rejects bodies on no-body routes and bounds creator-info queries", async () => {
    ...
    const oversizedQuery = await getTikTokCreatorInfo(
      new Request(
        `https://clipstitchr.test/api/studio/publishing/integrations/tiktok/creator-info?integrationId=${"a".repeat(4_096)}`,
      ),
    );
  ...
  it("consumes callback data through the service and redirects without echoing it", async () => {
    ...
    const response = await completeOAuthCallback(
      new Request(
        `https://clipstitchr.test/api/studio/publishing/oauth/tiktok/callback?code=private-code&state=${state}`,
      ),
    ...
    expect(response.headers.get("location")).toBe(
      "https://clipstitchr.test/dashboard/studio/publishing/integrations?connection=connected",
    );
  ...
  it("consumes provider denial once without forwarding its description", async () => {
    ...
    const response = await completeOAuthCallback(
      new Request(
        `https://clipstitchr.test/api/studio/publishing/oauth/instagram/callback?error=access_denied&error_description=private-provider-detail&state=${state}`,
      ),
  ...
  it("fails closed on duplicate callback fields and never leaks thrown secrets", async () => {
    const duplicate = await completeOAuthCallback(
      new Request(
        `https://clipstitchr.test/api/studio/publishing/oauth/tiktok/callback?code=one&state=${state}&state=${state}`,
      ),
      { params: Promise.resolve({ provider: "tiktok" }) },
    );
    ...
    const contradictory = await completeOAuthCallback(
      new Request(
        `https://clipstitchr.test/api/studio/publishing/oauth/tiktok/callback?code=one&error_description=private-detail&state=${state}`,
      ),
      { params: Promise.resolve({ provider: "tiktok" }) },
    );
