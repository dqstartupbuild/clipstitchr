...
function getClipKind(value: string | null) {
  return value === "clipr" ||
    value === "demo" ||
    value === "swapr" ||
    value === "ugc"
...
export async function GET(request: Request) {
  ...
  const clips = await convex.query(api.cliLibrary.listCliClips.listCliClips, {
    kind: getClipKind(url.searchParams.get("kind")),
