...
import { GET as getPublishingAnalytics } from "@/app/api/publishing/analytics/route";
import { GET as getPublishingCalendar } from "@/app/api/publishing/calendar/route";
import { POST as checkPublishingCompatibility } from "@/app/api/publishing/media/compatibility/route";
import { POST as cancelPublishingPost } from "@/app/api/publishing/posts/[postId]/cancel/route";
import { POST as retryPublishingPost } from "@/app/api/publishing/posts/[postId]/retry/route";
import { GET as getPublishingPost } from "@/app/api/publishing/posts/[postId]/route";
import {
  GET as getPublishingPosts,
  POST as createPublishingPost,
} from "@/app/api/publishing/posts/route";
...
describe("publishing post API routes", () => {
  ...
  it("requires Clerk authentication before reading a compatibility body", async () => {
    ...
    const response = await checkPublishingCompatibility(
      new Request(
        "https://clipstitchr.test/api/publishing/media/compatibility",
        { method: "POST" },
      ),
    );
  ...
  it("fails closed when owner-scoped durable media resolution finds nothing", async () => {
    ...
    expect(mocks.resolvePublishingApiMedia).toHaveBeenCalledWith(media);
  ...
  it("rejects a stale client media revision before creating service work", async () => {
    const response = await createPublishingPost(
      jsonRequest("https://clipstitchr.test/api/publishing/posts", {
        ...createRequest,
  ...
  it("rejects destination media errors before creating service work", async () => {
    ...
    const response = await createPublishingPost(
      jsonRequest(
        "https://clipstitchr.test/api/publishing/posts",
        createRequest,
      ),
    );
  ...
  it("forwards the safe post request with its exact immutable manifest and no URL", async () => {
    ...
    expect(forwarded).toEqual({
      ...
      body: { ...createRequest, resolvedMedia: manifest },
      method: "POST",
      path: "/v1/posts",
    });
  ...
  it("bounds list, calendar, and analytics queries before proxying", async () => {
    const invalidPosts = await getPublishingPosts(
      new Request(
        "https://clipstitchr.test/api/publishing/posts?status=failed&status=queued",
      ),
    );
    const invalidCalendar = await getPublishingCalendar(
      new Request(
        "https://clipstitchr.test/api/publishing/calendar?from=2026-08-01T00%3A00%3A00.000Z&to=2026-08-08T00%3A00%3A00.000Z&timeZone=Not%2FAZone",
      ),
    );
    const invalidAnalytics = await getPublishingAnalytics(
      new Request(
        "https://clipstitchr.test/api/publishing/analytics?range=365d",
      ),
    );
  ...
  it("preserves service rate-limit status and retry timing", async () => {
    ...
    const response = await getPublishingAnalytics(
      new Request(
        "https://clipstitchr.test/api/publishing/analytics?range=7d",
      ),
    );
  ...
  it("proxies bounded reads, retry, and cancel with exact service actions", async () => {
    await getPublishingPosts(
      new Request(
        "https://clipstitchr.test/api/publishing/posts?status=failed",
      ),
    );
    await getPublishingPost(
      new Request("https://clipstitchr.test/api/publishing/posts/post_123"),
    ...
    await getPublishingCalendar(
      new Request(
        "https://clipstitchr.test/api/publishing/calendar?from=2026-08-01T00%3A00%3A00.000Z&to=2026-08-08T00%3A00%3A00.000Z&timeZone=America%2FDetroit",
      ),
    );
    await getPublishingAnalytics(
      new Request(
        "https://clipstitchr.test/api/publishing/analytics?range=30d",
      ),
    );
