import { Check, ClipboardList } from "lucide-react";

const benefits = [
  "A clear creator handoff",
  "Reusable source clips",
  "No invented proof",
];

export function AppUgcBriefBuilderHero() {
  return (
    <section className="marketing-grid-bg border-b border-border px-6 py-20 md:py-28">
      <div className="mx-auto max-w-6xl">
        <div className="max-w-4xl">
          <p className="marketing-eyebrow">
            <ClipboardList aria-hidden className="h-3.5 w-3.5" />
            UGC planning tool for app ads
          </p>
          <h1 className="marketing-heading mt-6 text-5xl text-text-primary md:text-7xl">
            UGC Ad Brief Builder for Apps
          </h1>
          <p className="mt-6 max-w-3xl text-lg leading-8 text-text-secondary">
            Turn your app, audience, product moment, and honest proof into a
            creator brief that asks for footage you can actually reuse. Copy
            the full handoff when it feels right.
          </p>
          <div className="mt-8 flex flex-wrap gap-4 text-sm font-semibold text-text-tertiary">
            {benefits.map((benefit) => (
              <span className="inline-flex items-center gap-2" key={benefit}>
                <Check aria-hidden className="h-4 w-4 text-accent" />
                {benefit}
              </span>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
}
