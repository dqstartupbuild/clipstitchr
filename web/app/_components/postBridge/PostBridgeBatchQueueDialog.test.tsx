...
vi.mock("@/lib/clipstitchr/client/queuePostBridgeBatchItems", () => ({
  queuePostBridgeBatchItems: vi.fn(),
}));

const ACCOUNT: PostBridgeSocialAccount = {
  id: 12,
  platform: "instagram",
  username: "creator",
};
...
describe("PostBridgeBatchQueueDialog", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    vi.mocked(fetchPostBridgeAccountOptions).mockResolvedValue({
      accounts: [ACCOUNT],
  ...
  it("keeps active progress visible when a parent passes a fresh items array", async () => {
    ...
    setDialogState({ completedCount: 1, progress: 0.6, status: "queueing" });
    const tree = PostBridgeBatchQueueDialog({
      items: [...firstItems],
      onClose: vi.fn(),
    });
    ...
    expect(fetchPostBridgeAccountOptions).toHaveBeenCalledOnce();
    expect(fetchPostBridgeAccountOptions).toHaveBeenCalledWith("product_1");
  ...
  it("starts only one batch when the queue button is pressed twice", async () => {
    ...
    vi.mocked(queuePostBridgeBatchItems).mockReturnValue(pendingQueue);
    setDialogState();
    const tree = PostBridgeBatchQueueDialog({
    ...
    expect(queuePostBridgeBatchItems).toHaveBeenCalledOnce();
