import React from "react";
import { renderToStaticMarkup } from "react-dom/server";
import { describe, expect, it, vi } from "vitest";
import { SettingsSocialPublishingProductConfigDialog } from "@/app/_components/settings/SettingsSocialPublishingProductConfigDialog";
import type { ProductProfile } from "@/lib/clipstitchr/types/ProductProfile";

vi.mock("convex/react", () => ({
  useMutation: () => vi.fn(async () => undefined),
}));

function createProduct(
  overrides: Partial<ProductProfile> = {},
): ProductProfile {
  return {
    audienceDetails: "Creators",
    createdAt: "2026-07-09T00:00:00.000Z",
    id: "product_1",
    inferredPainPoints: [],
    name: "Launch Kit",
    productDetails: "Launch faster",
    updatedAt: "2026-07-09T00:00:00.000Z",
    ...overrides,
  };
}

describe("SettingsSocialPublishingProductConfigDialog", () => {
  it("renders as a modal dialog with product config copy", () => {
    const markup = renderToStaticMarkup(
      <SettingsSocialPublishingProductConfigDialog
        hasApiKey={false}
        isDisabled={false}
        products={[createProduct()]}
        onClose={vi.fn()}
      />,
    );

    expect(markup).toContain('role="dialog"');
    expect(markup).toContain("Zernio accounts");
    expect(markup).toContain("Save your Zernio key first");
    expect(markup).toContain("Close Zernio account config");
  });
});
