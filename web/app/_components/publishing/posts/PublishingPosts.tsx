...
export function PublishingPosts({
  ...
  const resource = usePublishingResource(
    (signal) => getPublishingPosts(status, signal),
    status,
  );
  ...
  return (
    <section className="publishing-view" aria-labelledby="publishing-posts-title">
      <PublishingViewHeader
        action={
          <Link className="publishing-primary-action" href="/dashboard/publishing/compose">
            Create post
          </Link>
        }
        description="Open a draft, scheduled destination, or provider result before deciding what to do next."
        title="Posts"
      ...
      {resource.error ? (
        ...
        <PublishingStateMessage
          action={
            <Link className="publishing-text-action" href="/dashboard/publishing/compose">
              Create a post
            </Link>
          }
        ...
        <div className="publishing-posts-layout" aria-busy={resource.isLoading}>
          ...
          {initialPostId ? (
            <PublishingPostDetail id={initialPostId} key={initialPostId} />
          ) : (
            <PublishingStateMessage
