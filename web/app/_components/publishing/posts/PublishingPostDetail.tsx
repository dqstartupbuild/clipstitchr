...
import { usePublishingPostDetailActions } from "./usePublishingPostDetailActions";
...
export function PublishingPostDetail({ id, productId }: PublishingPostDetailProps) {
  const resource = usePublishingResource(
    (signal) => getPublishingPost(id, productId, signal),
    productId ? `${productId}:${id}` : null,
  );
  const actions = usePublishingPostDetailActions(id, productId);
  const post = actions.postOverride ?? resource.data?.post ?? null;
  ...
  return (
    <aside className="publishing-post-detail" aria-label="Post details">
      ...
      <div className="publishing-post-detail-actions">
        {post.status === "published" && post.providerPublicationIds.length ? (
          <button
            disabled={actions.isRefreshingAnalytics || actions.isActing || actions.confirmation !== null}
            onClick={() => void actions.refreshAnalytics()}
            type="button"
          >
            {actions.isRefreshingAnalytics ? "Contacting provider…" : "Refresh provider analytics"}
          </button>
        ) : null}
        {post.canRetry ? (
          <button
            disabled={actions.isActing || actions.isRefreshingAnalytics}
            type="button"
            onClick={() => actions.setConfirmation("retry")}
          >
        ...
        {post.canCancel ? (
          <button
            disabled={actions.isActing || actions.isRefreshingAnalytics}
            type="button"
            onClick={() => actions.setConfirmation("cancel")}
          >
            Cancel scheduled post
          </button>
        ) : null}
      </div>
      {actions.isRefreshingAnalytics ? (
        <p className="publishing-action-status" role="status">
          Asking the provider for its latest saved metrics.
        </p>
      ) : actions.analyticsResult ? (
        <p className="publishing-action-status" role="status">
          Provider metrics refreshed at {new Intl.DateTimeFormat(undefined, {
            dateStyle: "medium",
            timeStyle: "short",
          }).format(new Date(actions.analyticsResult.observedAt))}. {actions.analyticsResult.metrics.length} supported {actions.analyticsResult.metrics.length === 1 ? "metric was" : "metrics were"} saved.
        </p>
      ) : null}
      {actions.analyticsError ? (
      ...
      {actions.confirmation ? (
        <PublishingPostActionConfirmation
          actionLabel={
            actions.confirmation === "retry" ? "Retry this destination" : "Cancel schedule"
          }
          busy={actions.isActing}
          explanation={
            actions.confirmation === "retry"
          ...
          onCancel={() => actions.setConfirmation(null)}
          onConfirm={() => void actions.runAction()}
          tone={actions.confirmation === "cancel" ? "danger" : "neutral"}
        />
      ) : null}
      {actions.actionError ? (
        <p className="publishing-inline-error" role="alert">
