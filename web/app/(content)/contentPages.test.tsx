...
describe("content pages", () => {
  ...
  it("renders docs index and article pages with generated metadata", async () => {
    ...
    expect(indexMarkup).toContain("Publishing");
    expect(generateDocsArticleStaticParams()).toContainEqual({
      slug: "publishing",
    });
    ...
    const publishingArticleMarkup = renderToStaticMarkup(
      await DocsArticlePage({
        params: Promise.resolve({ slug: "publishing" }),
      }),
    );
    expect(publishingArticleMarkup).toContain("Connect an account");
    expect(publishingArticleMarkup).toContain(
      "Track queued, published, and failed posts",
    );
  });
  ...
  it("renders static content shells and legal pages", () => {
    ...
    expect(pricingMarkup).toContain("$399");
    expect(pricingMarkup).toContain("8,000 creation credits/month");
    expect(pricingMarkup).toContain("10 Clipr or Swapr videos each month");
    expect(pricingMarkup).toContain("Daily drafts for all 10 products");
    expect(pricingMarkup).toContain("1 stitch created on Starter or Pro");
    expect(pricingMarkup).toContain("Unlimited stitches");
    expect(pricingMarkup).toContain("Refill");
    expect(pricingMarkup).toContain("2,000 creation credits");
    ...
    expect(pricingMarkup).not.toContain("Clipr&#x27;s required scene photo");
    expect(pricingMarkup).not.toContain("10k Organic Views Challenge");
    expect(pricingMarkup).not.toContain("Choose Studio");
    expect(pricingMarkup).not.toContain("Start free");
    expect(privacyMarkup).toContain("Privacy Policy");
    expect(termsMarkup).toContain("Plan and Usage Terms");
    expect(termsMarkup).toContain("Billing and Automatic Renewal");
    expect(termsMarkup).toContain("Plan Changes and Cancellation");
    expect(termsMarkup).toContain("Failed Payments and Renewal Grace");
    expect(termsMarkup).toContain("72-hour payment grace period");
    expect(termsMarkup).toContain("Monthly Credits and Credit Refills");
    expect(termsMarkup).toContain("do not roll over");
    expect(termsMarkup).toContain("one-time $29 credit refill");
    expect(termsMarkup).toContain("replacement of that subscription");
    expect(termsMarkup).toContain("nontransferable");
    expect(termsMarkup).toContain("Payments are generally final");
    expect(termsMarkup).toContain("within 14 days after the charge");
    expect(termsMarkup).toContain("30 days");
    expect(termsMarkup).toContain("Support and Account Messages");
    expect(termsMarkup).toContain("support@followusai.com");
    expect(termsMarkup).toContain('href="mailto:support@followusai.com"');
    expect(privacyMarkup).toContain("support@followusai.com");
    expect(termsMarkup).toContain("10k Organic Views Challenge");
    expect(termsMarkup).toContain("Creating 10 stitches uses 100");
    expect(termsMarkup).toContain("does not use standalone photo credits");
